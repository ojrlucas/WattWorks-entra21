...
private static Connection con;
	...
	public static void salvarUnidade(Unidade uni) throws SQLException {
        try {
        	...
        	PreparedStatement stm = con.prepareStatement("INSERT INTO tb_unidades (descricao, cnpj, cadastrado_em, subgrupo_id, modalidade_id, entidade_id) VALUES (?,?,?,?,?,?)");
            stm.setString(1, uni.getDescricao());
            stm.setString(2, uni.getCnpj());
            stm.setObject(3, uni.getDataCadastro());
            stm.setInt(4, uni.getSubgrupoId());
            stm.setInt(5, uni.getModalidadeId());
            stm.setInt(6, uni.getEntidade_id());
            stm.execute();
            stm.close();
		}
